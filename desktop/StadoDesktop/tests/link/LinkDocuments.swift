...
/// The payloads the Link tests read, each copied from what `stado host link
/// <host> --json` actually printed on the day named in its comment. They live
/// together because two test classes read the same documents, and because a
/// payload edited to suit a test stops being evidence of anything.
enum LinkDocuments {
