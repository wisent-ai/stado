...
/// 2026-08-19 connectivity gap on `charless-mac-mini`: a six-minute silence
...
final class FleetLinkStoreTests: XCTestCase {
    func testDecodesTheWholeLinkDocument() throws {
        ...
        XCTAssertEqual(link.host, "charless-mac-mini")
    ...
    /// `stado host link ubuntu-server-rtx-pro-6000 --json` answered on
    ...
    func testALinkBlockThatWasNeverCollectedIsNotReportedRatherThanAPath() throws {
        ...
        XCTAssertEqual(link.host, "ubuntu-server-rtx-pro-6000")
    ...
    func testANullPathKindIsAlsoUnreported() throws {
        let link: HostLink = try XCTUnwrap(
            HostLinkStore.decode(
                from: """
                {"host": "lukasz-macbook", "beacon_age_seconds": 22, "ssh_reachable": true,
    ...
    func testANullBeaconAgeStaysNull() throws {
        let link: HostLink = try XCTUnwrap(
            HostLinkStore.decode(
                from: """
                {"host": "charless-mac-mini", "beacon_age_seconds": null, "ssh_reachable": false,
    ...
    /// `session` block.
    ///
    /// `charless-mac-mini` is an always-on box with nobody at its screen, and
    ...
    func testAnUnknownSessionKindIsNotReported() throws {
        let link: HostLink = try XCTUnwrap(
            HostLinkStore.decode(
                from: """
                {"host": "ubuntu-server-rtx-pro-6000", "beacon_age_seconds": 31,
    ...
    func testAnOpenSilenceMeasuresItsOwnElapsedTime() throws {
        ...
        let link: HostLink = try XCTUnwrap(
            HostLinkStore.decode(
                from: """
                {"host": "charless-mac-mini", "beacon_age_seconds": 450, "ssh_reachable": false,
                 "verdict": "silent",
                 "blockers": ["ssh connect Operation timed out", "100% ping loss over 2 packets"],
                 "silences": [{"host": "charless-mac-mini",
    ...
    func testAnOpenSilenceBecomesAPostureDecisionRoutedAtThatHost() throws {
        ...
        let link: HostLink = try XCTUnwrap(
            HostLinkStore.decode(
                from: """
                {"host": "charless-mac-mini", "beacon_age_seconds": 366, "ssh_reachable": false,
        ...
        XCTAssertEqual(decision.host, "charless-mac-mini")
        XCTAssertEqual(decision.destination, .hosts)
        XCTAssertEqual(decision.tone, .danger)
        XCTAssertEqual(
            decision.title,
            "charless-mac-mini has been silent for 6 min",
        ...
        XCTAssertEqual(
            ...
            "stado host link charless-mac-mini --json",
    ...
    func testOpenSilencesAreOrderedLongestQuietFirst() throws {
        ...
        let brief = try XCTUnwrap(Self.openSilenceLink(host: "ubuntu-server-rtx-pro-6000", quietFor: 320))
        let long = try XCTUnwrap(Self.openSilenceLink(host: "charless-mac-mini", quietFor: 900))
        let posture = FleetPosture(snapshot: snapshot, report: nil, links: [brief, long])

        XCTAssertEqual(
            posture.openSilences.map(\.link.host),
            ["charless-mac-mini", "ubuntu-server-rtx-pro-6000"]
        )
    }
    ...
    func testTheQuotedCommandIsTheInvocation() {
        XCTAssertEqual(
            HostLinkStore.linkArguments(host: "charless-mac-mini"),
            ["host", "link", "charless-mac-mini", "--json"]
        )
        XCTAssertEqual(
            HostLinkStore.commandLine(host: "charless-mac-mini"),
            "stado host link charless-mac-mini --json"
        )
    }
    ...
    /// Copied byte for byte from `stado host link ubuntu-server-rtx-pro-6000
    ...
    /// `charless-mac-mini` as `stado host link charless-mac-mini --json` answers
    ...
    private static let headlessDocument = """
    {
