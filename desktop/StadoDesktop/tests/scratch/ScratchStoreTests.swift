...
final class ScratchStoreTests: XCTestCase {
  ...
  func testALeaseTakenThroughTheStoreIsTheLeaseTheHostReportsAndLoses() async throws {
    ...
    let build = URL(fileURLWithPath: #filePath)
      ...
      .appendingPathComponent(".build/scratch-test-runs")
    try FileManager.default.createDirectory(at: build, withIntermediateDirectories: true)
    store.form.root = build.appendingPathComponent(UUID().uuidString).path
    ...
    defer {
      if !destroyed {
        product.destroy(name: receipt.name, host: host.target)
      }
    }
    ...
    let registryBefore = try Data(contentsOf: URL(fileURLWithPath: receipt.registryPath))
    store.form.name = receipt.name
    await store.create(host: host.target)
    XCTAssertNotNil(store.refusal, "a duplicate lease must be refused")
    XCTAssertEqual(
      try Data(contentsOf: URL(fileURLWithPath: receipt.registryPath)),
      registryBefore,
      "a refused duplicate must not replace the existing lease registry"
    )
    ...
    destroyed =
      gone.account.exists == false && gone.home.exists == false && gone.record.exists == false
...
private struct RealScratch {
  ...
  func leasableHost() throws -> Host {
    ...
    guard
      let chosen = leasable.first(where: { ($0["target"] as? String) != Self.localName() })
        ?? leasable.first
    else {
  ...
  func destroy(name: String, host: String) {
    do {
      _ = try json(["scratch", "destroy", name, "--host", host, "--json"])
    } catch {
      XCTFail("scratch lease cleanup failed: \(error)")
    }
  }
