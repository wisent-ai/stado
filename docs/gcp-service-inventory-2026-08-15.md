...
The 2026-08-17 live export contains 2,674 direct locators for `https://storage.googleapis.com/wisent-images-bucket/`: 1,664 `Character.imageUrl`, 927 `Character.videoUrl`, 21 `ProfilePublic.imageUrl`, and 62 `Room.imageUrl` values. GCS listing proves every referenced object name exists and records 3,233,160,110 bytes (3.011 GiB) with generation, size, MD5, CRC32C and content type, but authenticated body reads still return HTTP 403 while project billing is absent. These are active application data references, not inert archive metadata.
...
| Swiatowid PTY relay | Cloud Run `swiatowid-pty-relay`, seven revisions, five current source-deploy image records, source bucket, relay token | Generic Oko Desktop/`oko-cli` relay clients exist, but no current source/configuration selects either deployed GCP URL | This is the only Cloud Run service resource in the live asset inventory. Its control-plane status is `Ready`, but request logs contain no requests in the last 90 days and runtime logs show repeated startup aborts because detached billing prevents the Secret Manager read. Publisher use requires a non-empty configured `STADO_PTY_RELAY_URL`; viewer use can receive a dynamic URL from `oko_live_sessions`. This Mac has no configured publisher URL, and no repository binding to the GCP endpoint was found. |
...
| Billing analytics | BigQuery dataset `billing_export` and table `gcp_billing_export_v1_017364_D3B657_F207B5` | Former Stado GCP billing-health reader; Google-managed billing export remains the writer | Historical GCP gross cost, credits, net cost and burn history only. Active Stado billing configuration selects `azure` and no current Wisent runtime consumes this table. |
...
|---|---|---|
| BigQuery `billing_export` | Google-managed billing export writer; former Stado billing-health reader | Dataset present; no current Wisent consumer. |
| BigQuery `gcp_billing_export_v1_017364_D3B657_F207B5` | Historical GCP gross-cost, credit, net-cost and burn calculations | Optional financial archive, not a runtime or Microsoft migration dependency. |
