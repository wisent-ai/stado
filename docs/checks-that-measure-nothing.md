...
One defect shape has now been found forty times in this repository, in
forty different subsystems, inside about two days. The twentieth was
...
| 39 | `claimability` in `src/cli/release_submit.rs` (#405) | a publication whose `free_slots` sums to zero is the host refusing work — and a missing table is silence, which the same function is careful not to read as "no" | **the difference between an empty table and an exhausted one.** `build_capacity_dict_per_card` returns an empty map for every host whose `gpu_type` is absent or `cpu`, so both Macs of this fleet have always published `"free_slots": {}` — while claiming and running CPU work, including every release-worker job that has ever built a release here. Reading `{}` as a positive refusal left `darwin-arm64` with no eligible builder from the moment #349 landed: every product release refused with `no live fleet builder can CLAIM release_platform darwin-arm64 ... 0 free slot(s), no blocker declared`, which is the selector quoting its own inference back as the host's statement. Braces are not a sentence: an empty table is now `Unstated` like a missing one, an exhausted table with keys still refuses, declared blockers still refuse, and the wedged GPU shape #349 was written for — fresh publication, `free_vram_gb` zero, no slot for the VRAM the host holds — is refused by naming `total_vram_gb` instead |
