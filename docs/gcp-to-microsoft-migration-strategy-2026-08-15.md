...
### Brama recovered from a crash loop, and where the edge repair now stops — 2026-08-18

Trying to read the Cloudflare zone through Weles uncovered a chain of latent faults, each one a declaration that nothing checked against the world until a restart forced the question. In order found:

1. **The Weles launcher overrode the agent model.** `~/.stado/bin/weles-api-launcher` sourced three env files and then hardcoded `export WELES_AGENT_MODEL=best`, so every browser job died with "must be the exact supported Brama alias weles/agent/primary" no matter what the env files or the launchd unit said. The line is now a default, and the generator lives in the Weles repository, where the same change belongs.
2. **Brama itself was crash-looping**, and had been serving only because an older configuration was already loaded — the exact hazard of cycling a service whose config was never re-validated. Its control config `~/.stado/brama-28b-control.json` carried the alias **`-best`** where the closed set requires `best`, in both `allowed_models` and `model_aliases`, with an otherwise correct route. One stray hyphen stopped every model call in the company: browser automation, product chat, evaluation, embeddings and moderation all route through this gateway. The same corruption sat in this repository's `deploy/local/stado.config.json`, and is fixed there too.
3. **Brama could not build its client identities.** After the alias fix it bound port 8080 and exited with "BRAMA_MODEL_ROUTER_CLIENT_IDENTITIES must give `weles` its exact required alias set": the launcher acquires each client's router token through the Skarbiec capability broker, and `~/.stado/capability-routes.json` granted provider and Apple items but not one `*-model-router`. Adding exactly the two capabilities the server requires — `weles-model-router#token` and `wisent-backend-model-router#token` — produced a 20-client identity list containing `weles`.

Brama now runs stably: same pid across a 90-second poll, `healthz` 401 (alive, authenticated routes only), and the log records "routing request accepted". That restores model routing for every Wisent client, which was down for reasons unrelated to this migration.

**Where it stops:** the Weles browser job now reaches Brama and is refused — `model router 403 authorization_error` for client `weles`. Brama's log also shows Skarbiec capability redemptions denied and an OAuth refresh failure against subscription providers. That is the entitlements plane, a separate system from this migration, and the Cloudflare DNS repair sits behind it.

Measured state at the end of this pass: Azure media 3 of 3 sampled objects HTTP 200; the local gateway 200 for both a media path and the API; `com.wisent.cloudflared` and `com.wisent.bobloo-gateway` running and registry-adopted; `https://bobloo.com/health` still 502 from the Cloudflare edge, so the product API has no working public route. The 2,674 media rows point at the Azure host, which is why media works while the API does not.
