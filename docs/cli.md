...
| `stado host gui-automation grant-accessibility <target>` | Grant the installed, validly signed `/Applications/CuaDriver.app` Accessibility for the console user, or the approved remote-login user while the host is at the login window. Stado binds the row to the app's designated code requirement, keeps one owner-only backup of the prior TCC database, reads the written decision back, and never opens System Settings or a consent window. |
