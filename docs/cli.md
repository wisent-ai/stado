...
| `stado service catalog` | The preconfigured Wisent services this build ships — skarbiec, brama, weles, stado-agent, stado-object-api, stado-resolver — each with the program and arguments it deploys with. `service ensure <name>` and `service deploy <name>` resolve these by name when no flag and no registry declaration answers, so "run Weles here" is one command with nothing to configure: `stado service ensure weles --host <host> --reason <why>`. An explicit declaration always beats the catalog; the catalog is the default, never an override. The same list appears as one-click options in the console's Declare service sheet. |
