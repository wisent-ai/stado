...
| `stado service catalog` | The Wisent product-service catalog: installable entries (skarbiec, brama, weles, stado, oko, transcript-lake, wisent-backend) carry the program and arguments Stado deploys; product services that exist but lack a published host artifact (skarbiec-hub, wisent-customer-support, wisent-trade, trading-tools, trading-autonomy, growth-tactics) stay visible as unavailable with the missing contract named. Internal infrastructure (image-video-router, tunnels, health APIs, runners and product-private gateways) is deliberately absent. Desktop applications such as Tama and harnesses such as Lem belong to their own install surfaces, not the service catalog. Stado is one composite service (`stado local-control-plane`): API listener, scheduler and worker stay internal instead of becoming three operator choices. `service ensure <name>` and `service deploy <name>` resolve these by name when no flag and no registry declaration answers, so "run Weles here" is one command with nothing to configure: `stado service ensure weles --host <host> --reason <why>`. An explicit declaration always beats the catalog; the catalog is the default, never an override. The same list appears as one-click options in the console's Declare service sheet. |
...
- **Marker against the socket table** (`reconciliation` on each marker,
  `stale_markers` in the summary). On `charless-mac-mini` the marker
...
- **Marker against the registry** (`declared_url` and
  ...
  still carries a declared endpoint for it. On `charless-mac-mini` the marker
...
Shell scripts are counted, not rowed. charless-mac-mini carries 1393 of them in
...
stado host software charless-mac-mini --json
...
```bash
stado host gates charless-mac-mini
```
...
stado host reclaim charless-mac-mini --apply \
  --reason 'queue agent has published disk_pressure_unresolved since 08:10'
```

```
DRY RUN — nothing on charless-mac-mini is deleted. Re-run with --apply --reason <text> to remove what follows.
...
|---|---|
| No report | `charless-mac-mini has never reported what software it runs…`. This is the state that used to print as `observed=unreported` beside exit 0. |
...
```bash
stado release logs brama --target charless-mac-mini --version 0.2.27 --stream err --lines 40
```
...
```bash
stado release doctor brama --target charless-mac-mini
```
...
next: stado release logs brama --target charless-mac-mini --version 0.2.27 --stream err
```
...
charless-mac-mini  46748  stado          Fri Aug 14 09:12:33 2026  /Users/charles/.stado/bin/stado agent
```
...
```bash
stado service ensure weles-api --host charless-mac-mini \
...
charless-mac-mini  weles-api  com.wisent.stado.service.weles-api     system  created  908
audit record service_audit/charless-mac-mini/20260818T142530.114203Z-com.wisent.stado.service.weles-api.json
```
