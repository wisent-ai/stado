...
| `stado host disk <target>` | Disk usage plus the registry cleanup policy and the janitor's own state: last pass, bytes freed, next scheduled pass. Also names local APFS snapshots so eligible Time Machine snapshots can be distinguished from OS-update snapshots that reclamation refuses. |
...
| `stado host reclaim <host> [--dry-run\|--apply --reason TEXT]` | Reclaim disk through eight declared stages: registry janitor, build scratch, terminal queue workdirs, foreign macOS home trees on Linux, obsolete delivered versions, exact rebuildable caches, macOS Chromium code-sign clones, and eligible local Time Machine snapshots. Previews by default; `--apply` needs `--reason`, stops target-aware stages at the registry free-space target, protects live paths, and appends an audit record on the host. |
...
| `stado host gui-automation status <target>` | Report the macOS GUI-automation state: console owner, autologin password file, Remote Management settings, CuaDriver installation, and the per-user Accessibility decision read from TCC. |
| `stado host gui-automation enable <target>` | Reconcile the complete CuaDriver lifecycle on a registry-managed Apple Silicon Mac. Stado downloads the pinned universal release, verifies its SHA-256 and nested Apple signature, checks bundle id and version, atomically installs and registers `/Applications/CuaDriver.app`, publishes the login user's CLI symlink, and grants Accessibility. A second run reuses the verified app and reasserts the same TCC decision. No installer or helper script is copied to the host. |
| `stado host gui-automation grant-accessibility <target>` | Reassert Accessibility for an already installed, validly signed `/Applications/CuaDriver.app`. Stado binds the row to the app's designated code requirement, keeps one owner-only backup of the prior TCC database, reads the written decision back, and never opens System Settings or a consent window. |
| `stado host gui-automation disable <target> [--bundle ID]` | Revert GUI automation: remove autologin state, disable Remote Management and legacy VNC, revoke the named bundle's Accessibility row, and remove the automation artifacts. Omitting `--bundle` leaves TCC untouched rather than guessing an identity. |
...
| `stado host remove-file <target> <path>` | Remove one file from the host through the audited channel, with the guards a bare `rm` over ssh does not have. The path must be absolute and live under `$HOME/Library/LaunchAgents` or `$HOME/.stado` of the approved account, must be a regular file owned by that account, and must not be a symlink — a symlink under an allowed root can point anywhere. A directory is refused, a missing file is reported `absent` rather than invented into a success, and a system path is refused with the privileged command that could remove it named. Guards run on the host side, because the file is what the host says it is. This is the delete half `service retire` deliberately does not have, and the primitive `service remove` composes. |
| `stado service remove <unit> --host <host>` | Remove a service entirely: stop it on the host, drop its registry entry, and delete its declared unit file, in that order. The file path comes from the declaration, never from operator words. A unit that cannot be stopped refuses before anything moves and keeps its file; a file the channel may not delete leaves the service retired and says so, with the privileged command that could remove it. The `--json` answer is one document carrying the new registry generation and the file's own status (`removed`, `absent`, `refused`, `failed`) with the refusal's reason verbatim. |
| `stado service catalog` | The installable service subset generated from the canonical `wisent-products/catalog/products.yml` — skarbiec, brama, weles, stado, oko and transcript-lake — each with the program and arguments it deploys with. Internal infrastructure (routers, tunnels, health APIs, runners and product-private gateways) is deliberately absent: the catalog is for products an operator chooses, not every process the fleet happens to run. Stado is one composite service (`stado local-control-plane`): API listener, scheduler and worker stay internal instead of becoming three operator choices. `service ensure <name>` and `service deploy <name>` resolve these by name when no flag and no registry declaration answers, so "run Weles here" is one command with nothing to configure: `stado service ensure weles --host <host> --reason <why>`. An explicit declaration always beats the catalog; the catalog is the default, never an override. The same list appears as one-click options in the console's Declare service sheet. |
| `stado host sync-acquisition-scopes <target> <source>` | Register the checked-in Skarbiec acquisition-scope catalog on the host. The local catalog file is delivered into `$HOME/.stado/files` through the delivered-file channel — owner-only, checksummed on arrival — and an embedded fixed script derives the Ed25519 workload public key from the host's `weles-credential-workload-private.pem` (replacing an older key only after registration with its successor succeeds) and runs `skarbiec token-register-acquisitions --replace-capabilities` against the host's fleet vault, then prints the reconciled status. Nothing is installed on the host, and a refusal carries the remote's own words. |
...
A build delivered outside the release channel therefore reads `unmanaged` and that
...
`reclaim` is the write half, and it previews by default. Eight declared stages
run in this order, and nothing else runs:
...
3. `queue_workdirs` — terminal `wc-*` workdirs whose jobs are no longer queued
   or running, plus one-use `stado-bootstrap-*` trees. The queue's live job set
   is the keep-list, so age is not used as a substitute for ownership.
4. `foreign_home_trees` — `/Users/*` trees on Linux only. They can only be
   debris from a job or delivery carrying a hard-wired macOS path; the stage is
   a no-op on macOS and still refuses paths named by live processes.
5. `delivered_trees` — the version directories under `$HOME/.stado/services`,
   ...
   and every superseded delivery root the product catalog declares
   (`superseded_roots` in `stado-rs/data/products.json`). The mini carried 20
   obsolete `weles-worker` versions — 9.7 GiB — under the former delivery root
   `$HOME/.local/share/weles-worker`. The roots come from declarations so a
   delivery-path move is a data change. The live target, `current`, every
   product's newest tree, young trees and paths named by live processes stay.
6. `rebuildable_caches` — exact roots that their owning tools can recreate:
   Cargo git checkouts, Playwright browser bundles, npm content cache, completed
   Weles release probes and interrupted `.worker-download.*` staging trees.
   Active paths remain protected by the shared process snapshot. Apply stops
   sweeping these roots as soon as the registry's target free-space watermark
   is reached.
7. `chromium_clones` — `org.chromium.Chromium.code_sign_clone` under this
   ...
   that is killed leaves its clone behind. On the mini during the incident:
   108 clones occupied 158.3 GiB, while no product command previously accounted
   for them. The container's
...
`min_age_seconds` at one hour. The explicit stage covers disk-pressure recovery
before the janitor's next bounded pass.

8. `local_apfs_snapshots` — local Time Machine snapshots only, identified by
   the exact `com.apple.TimeMachine.YYYY-MM-DD-HHMMSS.local` format. A preview
   lists them. Apply deletes them one at a time only while free space remains
   below the registry target, and refuses OS-update or unknown snapshot names.

The safety rules are encoded in the command rather than left to whoever is at
the keyboard. Nothing outside declared roots is touched; no candidate path
comes from the operator. One process snapshot protects every held path.
Delivery trees keep `current` and the newest version; Chromium keeps its newest
clone. Filesystem stages use a one-day age gate except the one-hour clone,
interrupted-download and Cargo-checkout recovery paths. Target-aware stages
stop once the registry's requested free space exists. The same program runs in
preview and apply modes, with deletion behind the mode flag, so preview names
the exact candidates apply would consider.
...
queue_workdirs    2 GiB        2 GiB       0
foreign_home_trees 2 GiB       2 GiB       0
rebuildable_caches 2 GiB       6 GiB       12
chromium_clones   6 GiB        25 GiB      107
local_apfs_snapshots 25 GiB    25 GiB      0
...
APFS snapshots are accounted for explicitly. `host disk` names every local
snapshot. `host reclaim` may remove only eligible local Time Machine snapshots,
only in apply mode, and only until the registry target is reached. OS-update
snapshots and names outside the strict Time Machine format are refused and
remain visible in the report. `host gates` adds
`local_snapshots_unreclaimable` as a note, never a blocker, when snapshots exist
but none is eligible; their blocks are already inside the `used` figure from
`df`, and macOS publishes no size per snapshot, so Stado reports names and
counts without inventing byte figures.
