...
Probierz owns the combined `platform-matrix` journey in `stado-rs/tests/platform-matrix/`. It runs macOS through the managed host channel and Linux AMD64 in a pinned `linux/amd64` container from the same exact public commit. The Linux container verifies the digest of the published Skarbiec binary before using it. The journey runs the platforms one after another because both release checks use the same canonical test product and version.
