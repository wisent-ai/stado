...
At incident onset, the local `lukasz-macbook` agent started, initialized `JobStorage`, received the same GCS 403 before queue processing, exited, and was restarted by launchd roughly every 11–13 seconds.
...
| `charless-mac-mini` | local target | primary Weles browser worker; pinned placement |
| `ubuntu-server-rtx-pro-6000` | local GPU target | two RTX Pro 6000 slots for external/ComfyUI workloads |
| `lukasz-macbook` | local CPU target | two slots for CPU/TUI and Probierz remote jobs |
...
|---|---:|---|---|---:|---|---|---|
| `image-gen-comfyui-mig-62fj` | RUNNING | `us-central1-a` | `g2-standard-8` / 1 L4 | 200 | `wisent-ai/echo` | ComfyUI/Z-Image gateway and fallback image generation | high |
...
| `wisent-zimage-interp-jt` | TERMINATED | `us-central1-b` | `a2-highgpu-1g` / 1 A100 40 GB | 300 | `wisent-ai/echo` | Z-Image interpolation experiment | medium |
| `needher-watermark-a100-80gb-20260622` | RUNNING | `us-central1-c` | `a2-ultragpu-1g` / 1 A100 80 GB | 575 | `echo` + `needher-ai-web` | Needher watermark/content pipeline | medium |
| `sapiens2-bodyhorror` | TERMINATED | `us-central1-c` | `a2-ultragpu-1g` / 1 A100 80 GB | 575 | `echo` + `needher-ai-web` | Sapiens2/Needher content or watermark experiment | low |
...
| `content-platform-vm` | RUNNING | `us-west1-b` | `n2-standard-16` | 500 | `wisent-ai/echo` | long-lived Echo/Weles worker and recordings host | high |
...
|---|---|---|
| `us-central1-a/image-gen-comfyui-mig` | `echo` | L4 ComfyUI gateway; autoscaling declaration min 1, max 3 |
...
|---|---|---|---|
| `gs://stado` | `wisent-compute`, `echo`, `wisent-tools`, `wisent-tester/probierz` | canonical queue, lifecycle records, capacity, schedules, config, artifacts, Probierz inputs/results | current default and deployment |
...
| `gs://wisent-gcp-pipeline` | `echo`, `needher-ai-web`, `wisent-backend` | generated images, Needher watermarks and feed snapshots, LoRA training output, ComfyUI batch inputs/results, model staging | code |
| `gs://wisent-images-bucket` | `echo`, `wisent-backend`, `wisent` | SmoothMix models and videos, LoRAs, control vectors, optimization checkpoints | code |
...
| `us-central1-docker.pkg.dev/wisent-480400/smoothmix-comfyui` | `echo` | ComfyUI/SmoothMix GPU workload images |
...
| `comfyui-static-ip` | regional static address | `echo` | stable external ComfyUI endpoint |
| `allow-comfyui-8188` | firewall rule | `echo` | public and health-check access to ComfyUI port 8188 |
| `image-gen-comfyui-health` | health check | `echo` | `/system_stats` auto-healing check |
...
| `image-gen-comfyui` | custom image family | `echo` | baked ComfyUI, Z-Image, models, and LoRAs |
...
| `droid-441@wisent-480400.iam.gserviceaccount.com` | `wisent-backend`, `echo` | backend MIG templates and ComfyUI template |
...
| `wisent-ai/echo` | `wc submit`, GCS output/DONE markers, ComfyUI gateway, GCP image assets | new renders and their status/artifacts unavailable |
...
Local clones and worktrees are deduplicated by Git remote. For example, `echo`, deployment checkouts, and former Content Platform worktrees all represent `wisent-ai/echo`, not separate production repositories.
