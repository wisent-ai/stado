...
mod tests {
    ...
    const DIGEST: &str = "25a83915167b22f1bd53323777fbc5a10833bbe2f520ffa4abafa1cd6240986b";

    fn row(name: &str, version: &str, provenance: &str) -> HostSoftware {
        HostSoftware {
            name: name.to_string(),
            path: format!("/Users/charles/.stado/bin/{name}"),
    ...
    fn fresh(host: &str, rows: Vec<HostSoftware>) -> Report {
        Report {
            host: host.to_string(),
            rows,
            scripts: 553,
            freshness: Freshness::Fresh(Observation::now(
                report_fact(host),
                host,
                OBSERVED,
                "reported=1 scripts=553 names=skarbiec",
            )),
        }
    }
    ...
    /// The line the whole change is against: a host that says nothing must not
    /// read as a host that is fine.
    #[test]
    fn a_host_that_never_reported_is_a_failure() {
        let finding = judge(&Report::never("charless-mac-mini"), &BTreeMap::new(), None);
        assert!(finding.failed);
        assert!(finding.sentences[0].contains("never reported what software it runs"));
        assert!(finding.sentences[0].contains("stado host software charless-mac-mini"));
    }
    ...
    fn an_unmanaged_program_fails_and_names_the_disagreement() {
        let report = fresh("charless-mac-mini", vec![row("skarbiec", "0.2.3", UNMANAGED)]);
        let finding = judge(&report, &declared(&[("skarbiec", "0.1.3")]), None);
        assert!(finding.failed);
        let sentence = &finding.sentences[0];
        assert!(sentence.starts_with("charless-mac-mini runs skarbiec 0.2.3 at "));
        assert!(sentence.contains("matches no release artefact Stado published"));
        assert!(sentence.contains("the fleet declares 0.1.3"));
    }

    /// A released program at the declared version is the only clean state.
    #[test]
    fn a_released_program_at_the_declared_version_passes() {
        let report = fresh("charless-mac-mini", vec![row("skarbiec", "0.1.3", RELEASE)]);
        let finding = judge(&report, &declared(&[("skarbiec", "0.1.3")]), None);
        assert!(!finding.failed, "{:?}", finding.sentences);
        assert!(finding.sentences.is_empty());
    }

    /// A program nothing declares is reported and does not decide the gate:
    /// eleven dated backup copies of `stado` must not fail this forever.
    #[test]
    fn an_undeclared_program_is_reported_without_failing_the_gate() {
        let report = fresh(
            "lukasz-macbook",
            vec![
                row("skarbiec", "0.1.3", RELEASE),
                row("stado.0.7.2-backup-20260816", "0.7.2", UNMANAGED),
            ],
        );
        let finding = judge(&report, &declared(&[("skarbiec", "0.1.3")]), None);
        assert!(!finding.failed, "{:?}", finding.sentences);
    ...
    /// A declared program the report does not name is unconfirmed, and
    /// unconfirmed is a failure — the report is the confirmation.
    #[test]
    fn a_declared_program_missing_from_the_report_fails() {
        let report = fresh("charless-mac-mini", Vec::new());
        let finding = judge(&report, &declared(&[("weles-worker", "0.5.1")]), None);
        assert!(finding.failed);
        assert!(finding.sentences[0].contains("names no weles-worker program at all"));
    }

    /// The rollout binary lives under the product install root, so it is matched
    /// on its declared path and not only on its name.
    #[test]
    fn the_product_binary_is_judged_against_the_desired_release() {
        let mut brama = row("brama", "0.2.16", UNMANAGED);
        brama.path = "/Users/charles/.stado/services/brama/bin/brama".to_string();
        let report = fresh("charless-mac-mini", vec![brama]);
        let product = ProductBinary {
            name: "brama".to_string(),
            path: "/Users/charles/.stado/services/brama/bin/brama".to_string(),
            desired: Some("0.2.27".to_string()),
        };
        let finding = judge(&report, &BTreeMap::new(), Some(&product));
        assert!(finding.failed);
        assert!(finding.sentences[0].contains("the fleet declares 0.2.27"));
    }

    /// A stale report is history, and history must never be read as the present
    /// — the twelve-day outage in one assertion.
    #[test]
    fn a_stale_report_fails_even_when_every_program_agrees() {
        let mut report = fresh("charless-mac-mini", vec![row("skarbiec", "0.1.3", RELEASE)]);
        report.freshness = Freshness::Stale(Observation::now(
            report_fact("charless-mac-mini"),
            "charless-mac-mini",
            OBSERVED,
            "reported=1 scripts=0 names=skarbiec",
        ));
        let finding = judge(&report, &declared(&[("skarbiec", "0.1.3")]), None);
        assert!(finding.failed);
        assert!(finding.sentences[0].contains("past the window an observation speaks for"));
    }

    /// A read that could not happen is neither a clean report nor a silent one.
    #[test]
    fn a_refused_read_fails_in_the_channels_own_words() {
        let mut report = fresh("charless-mac-mini", Vec::new());
        report.freshness = Freshness::Fresh(Observation::now(
            report_fact("charless-mac-mini"),
            "charless-mac-mini",
            UNVERIFIED,
            "reported=0 scripts=0 names= ssh: connect to host port 22: Operation timed out",
        ));
        let finding = judge(&report, &BTreeMap::new(), None);
        assert!(finding.failed);
        assert!(finding.sentences[0].contains("Operation timed out"));
    }

    /// The wire format, including the one field that may contain a space.
    #[test]
    fn the_reporter_output_round_trips_through_the_record() {
        let stdout = concat!(
            "# host software report at 2026-08-19T00:19:44Z\n",
            "software name=skarbiec version=0.2.1 sha256=25a8 provenance=unmanaged",
            " path=/Users/lukaszbartoszcze/.stado/bin/skarbiec\n",
            "software name=weles version=unknown sha256=unknown provenance=release",
            " path=/Users/charles/Application Support/weles\n",
            "report reported=2 scripts=77\n"
        );
        let (rows, scripts) = parse(stdout);
        assert_eq!(scripts, 77);
        assert_eq!(rows.len(), 2);
        assert_eq!(rows[0].name, "skarbiec");
        assert_eq!(rows[0].version, "0.2.1");
        assert_eq!(rows[0].provenance, UNMANAGED);
        assert_eq!(rows[0].path, "/Users/lukaszbartoszcze/.stado/bin/skarbiec");
        assert_eq!(rows[1].path, "/Users/charles/Application Support/weles");
        assert!(rows[1].is_release());
        // The detail encoding is the storage format, so it has to survive the
        // same path the wire format does.
        let restored = HostSoftware::from_detail("weles", &rows[1].detail()).expect("row decodes");
        assert_eq!(restored, rows[1]);
    }

    /// A row the reporter could not complete is dropped, never defaulted: a
    /// fabricated `unmanaged` would accuse a host of carrying something unread.
    #[test]
    fn an_incomplete_row_is_dropped_rather_than_defaulted() {
        let (rows, _) = parse("software name=skarbiec version=0.2.1\nsoftware version=0.1.0\n");
        assert!(rows.is_empty());
    }

    /// The reporter is bound, never asked to decide scope.
    #[test]
    fn the_reporter_carries_the_declarations_it_is_given() {
        let script = reporter(
            &[
                (
                    "launchd".to_string(),
                    "/Library/LaunchDaemons/com.wisent.always-on.brama.plist".to_string(),
                ),
                (
                    "systemd".to_string(),
                    "/etc/systemd/system/image-video-router-release.service".to_string(),
                ),
            ],
            &[
                "/Users/charles/.stado/services/brama/bin/brama".to_string(),
                "/Users/charles/weles/bin/weles".to_string(),
            ],
        );
        ...
        // The reviewed body is what actually runs.
        assert!(script.contains("software name=%s version=%s"));
        // The bindings precede every use of them, or the reporter would fall
    ...
    /// The roster is what makes a removed program leave the report, in a store
    /// that merges and never deletes.
    #[test]
    fn the_roster_bounds_the_report_to_the_newest_names() {
        let host = "charless-mac-mini";
        let dropped = Observation::now(
            software_fact("weles-worker", host),
            host,
            OBSERVED,
            "version=0.5.1 sha256=abc provenance=release path=/Users/charles/weles/bin/weles",
        );
        let kept = Observation::now(
            software_fact("skarbiec", host),
            host,
            OBSERVED,
            format!("version=0.2.3 sha256={DIGEST} provenance=unmanaged path=/x/skarbiec"),
        );
        let roster = Observation::now(
            report_fact(host),
            host,
            OBSERVED,
            roster_detail(&["skarbiec"], 553),
        );
        let report = load_in(&[dropped, kept, roster.clone()], host);
        assert_eq!(report.rows.len(), 1);
        assert_eq!(report.rows[0].name, "skarbiec");
        assert_eq!(report.scripts, 553);
        assert_eq!(reported_hosts(&[roster]), vec![host.to_string()]);
    }
}
