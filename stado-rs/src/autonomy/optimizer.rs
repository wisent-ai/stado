...
mod tests {
    ...
    fn number<T>(raw: &str) -> T
    where
        T: std::str::FromStr,
        T::Err: std::fmt::Debug,
    {
        raw.parse().expect("static number")
    }

    fn at(raw: &str) -> chrono::DateTime<Utc> {
    ...
    #[test]
    fn queue_order_is_priority_then_earliest_deadline() {
        let mut low = Job::new("low", "work");
        low.priority = number("1");
        let mut late = Job::new("late", "work");
        late.priority = number("2");
        late.deadline_at = Some("2030-01-01T02:00:00Z".to_string());
        let mut early = Job::new("early", "work");
        early.priority = late.priority;
        early.deadline_at = Some("2030-01-01T01:00:00Z".to_string());

        assert_eq!(job_order(&late, &low), std::cmp::Ordering::Less);
        assert_eq!(job_order(&early, &late), std::cmp::Ordering::Less);
    }

    #[test]
    fn hard_deadline_rejects_candidate_and_prices_lateness() {
        let mut job = Job::new("deadline", "work");
        job.deadline_at = Some("2030-01-01T00:01:00Z".to_string());
        let offer = CapacityOffer {
            ...
            free_vram_gb: i64::default(),
            free_slots: true as i64,
            existing: true,
        };
        let policy = AutonomyPolicy {
            local_hourly_cost_usd: Some(number("1")),
        ...
        assert!(candidate.slo_penalty_usd > f64::default());
        assert!(candidate
            .rejected_reasons
            .iter()
            .any(|reason| reason.contains("deadline would be missed")));
    }
}
