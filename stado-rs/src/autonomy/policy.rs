...
#[cfg(test)]
mod tests {
    use super::super::model::Ownership;
    use super::*;

    fn protected_database() -> ResourceRecord {
        let mut resource = ResourceRecord::new(
            ProviderId::Gcp,
            "project",
            "database",
            "projects/project/instances/db",
            "db",
            chrono::Utc::now(),
        );
        resource.environment = Some("production".to_string());
        resource.ownership = Ownership::Owned;
        resource
    }

    #[test]
    fn protected_stateful_production_requires_both_explicit_exceptions() {
        let resource = protected_database();
        let mut policy = AutonomyPolicy {
            mode: AutonomyMode::EnforceOwned,
            ..AutonomyPolicy::default()
        };
        let mut rule = ResourceRule {
        ...
        assert!(
            !policy
                .authorize(&resource, ActionRisk::Destructive, true, None)
                .allowed
        );

        rule.allow_production_mutation = true;
        rule.allow_stateful_mutation = true;
