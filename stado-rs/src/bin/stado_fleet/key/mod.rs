...
///
/// Enumerates registry targets, not vault items. Each target's key is resolved
/// by exact credential item id; missing keys are reported as errors rather than
/// silently omitted. Renamed key items remain discoverable through the registry.
pub async fn ls() -> Result<bool, String> {
    ...
    for target in &registry.targets {
        ...
        let fingerprint_result = client.read_string(&id, "fingerprint").await;
        let key_type_result = client.read_string(&id, "key_type").await;

        let fingerprint = match fingerprint_result {
            Ok(Some(fp)) => fp,
            Ok(None) => "(no fingerprint field)".to_string(),
            Err(exc) => format!("(error: {})", exc),
        };

        let key_type = match key_type_result {
            Ok(Some(kt)) => kt,
            Ok(None) => "(no key_type field)".to_string(),
            Err(exc) => format!("(error: {})", exc),
        };

        shown.push(format!("{}\t{}\t{}", id, key_type, fingerprint));
    }

    if shown.is_empty() {
        println!("no targets in the registry");
    } else {
        for line in &shown {
            println!("{line}");
        }
...
pub async fn install(runner: &Runner, target: &str) -> Result<bool, String> {
    ...
    let connection =
        stado::deploy::host_channel::select_ssh_connection(target_entry, runner)
            .await
