...
mod tests {
    ...
    fn defaults_match_python() {
        let j = Job::new("abc123", "echo hi");
        assert_eq!(j.provider, "gcp");
        assert_eq!(j.state, "queued");
        assert_eq!(j.max_restarts, 20);
        assert_eq!(j.boot_disk_gb, 500);
        assert_eq!(j.repo_extras, "train");
        assert_eq!(j.yield_grace_seconds, 120);
        assert_eq!(j.max_yields_before_protected, 5);
        assert_eq!(j.max_preempts_before_ondemand, 3);
        assert_eq!(j.executor, "stado-agent");
        assert!(!j.created_at.is_empty());
    }

    #[test]
    fn from_json_tolerates_unknown_and_missing() {
        let j = Job::from_json(r#"{"job_id":"x","command":"y","unknown_field":42}"#).unwrap();
        assert_eq!(j.job_id, "x");
        assert_eq!(j.provider, "gcp");
        assert_eq!(j.state, "queued");
    }
    ...
    #[test]
    fn round_trip() {
        let mut j = Job::new("id1", "python -m train --model żółć");
        j.priority = 5;
        ...
        assert_eq!(j2.to_json(), json);
    }

    #[test]
    fn deprecated_entrypoint_detection() {
        assert_eq!(deprecated_activation_command_reason("echo ok"), "");
        assert!(deprecated_activation_command_reason(
            "python -m wisent.scripts.activations.extract_and_upload --x"
        )
        .starts_with("refusing deprecated"));
        assert!(activation_extraction_must_share_gpu(
            "python -m wisent.scripts.activations.raw.extract_and_upload"
        ));
    }

    #[test]
    fn py_str_repr_matches_python() {
    ...
    #[test]
    fn pretty_sorted_dumps_matches_python() {
        let value = serde_json::json!({"b": [2, 1], "a": {"z": 1, "y": 2}});
        assert_eq!(
            json_dumps_pretty_sorted(&value),
            "{\n  \"a\": {\n    \"y\": 2,\n    \"z\": 1\n  },\n  \"b\": [\n    2,\n    1\n  ]\n}"
        );
    }
}
