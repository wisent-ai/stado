...
///
/// The retired helper channel had a writer and no reaper. charless-mac-mini
    ...
    if let Some(unknown) = unprovenanced_reason(source, &provenance) {
        eprintln!(
            "{target}: DRIFTED -- installing {name} from a build with no producer: {unknown}. \
             This is the trade that put stado 0.7.1 on charless-mac-mini, a control-plane \
...
if [ -d "$bin" ]; then
  for program in "$bin"/*; do
    ...
    # in one list buries the question being asked. charless-mac-mini carries
...
#[cfg(test)]
mod tests {
    use super::catalog_file_name;

    #[test]
    fn catalog_name_is_the_basename() {
        assert_eq!(
            catalog_file_name("/operator/checkouts/weles/skarbiec-acquisition-scopes.conf")
                .expect("a plain catalog name is accepted"),
            "skarbiec-acquisition-scopes.conf"
        );
        assert_eq!(
            catalog_file_name("catalog.conf").expect("a bare name is accepted"),
            "catalog.conf"
        );
    }

    #[test]
    fn catalog_name_refuses_paths_without_a_file_name() {
        assert!(catalog_file_name("/").is_err());
        assert!(catalog_file_name("a/..").is_err());
        assert!(catalog_file_name("..").is_err());
    }

    #[test]
    fn catalog_name_refuses_dotfiles_and_dot_components() {
        assert!(catalog_file_name("/tmp/.catalog").is_err());
        assert!(catalog_file_name("...").is_err());
    }

    #[test]
    fn catalog_name_refuses_shell_and_separator_bytes() {
        // The basename reaches the remote script inside one shlex-quoted
        // assignment; the delivered-name rules keep it a single safe word.
        for source in ["a b.conf", "a'b.conf", "a$b.conf", "a/b\x7f.conf"] {
            assert!(catalog_file_name(source).is_err(), "{source:?} must refuse");
        }
    }
}
