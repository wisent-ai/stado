...
/// Complete one Skrzynka Gmail OAuth flow through the registry-selected Weles
/// host. This is intentionally narrower than a general browser-action command:
/// only Google's authorization endpoint and a loopback Skrzynka callback pass.
pub async fn weles_gmail_authorize(
    target: &str,
    ...
    let url = url::Url::parse(authorization_url)
        .map_err(|error| CmdError::usage(format!("invalid Gmail authorization URL: {error}")))?;
    if url.scheme() != "https" || url.host_str() != Some("accounts.google.com") {
        return Err(CmdError::usage(
            "Gmail authorization URL must use https://accounts.google.com".to_string(),
        ));
    }
    let redirect = url
        .query_pairs()
        .find_map(|(key, value)| (key == "redirect_uri").then(|| value.into_owned()))
        .ok_or_else(|| CmdError::usage("Gmail authorization URL has no redirect_uri".to_string()))?;
    let redirect = url::Url::parse(&redirect)
        .map_err(|_| CmdError::usage("Gmail redirect_uri is invalid".to_string()))?;
    if redirect.scheme() != "http"
        || redirect.host_str() != Some("127.0.0.1")
        || redirect.path() != "/v1/gmail/oauth/callback"
    {
        return Err(CmdError::usage(
            "Gmail redirect_uri must be the Skrzynka loopback callback".to_string(),
        ));
    }
    if !email.ends_with("@gmail.com") || email.contains(char::is_whitespace) {
        return Err(CmdError::usage(
            "Gmail account must be one @gmail.com address".to_string(),
        ));
    }
    let admission = crate::deploy::weles_capture::resolve_admission(target)
        .await
    ...
    let objective = format!(
        "Authorize Skrzynka Gmail access for {email} using the existing signed-in Google session. Approve only the requested scopes. Do not trigger device notifications, system consent windows, passkeys, SMS, phone prompts, or external approval; refuse if any is required. Success is the loopback callback page confirming completion."
    );
    let run_id = crate::deploy::weles_capture::run_action(
        &channel,
        "generic_keeper_task",
        json!({
    ...
    if json {
        ...
        println!("{target}: Gmail authorization completed through Weles run {run_id}");
    }
    Ok(())
}
