...
fn record_stream_services(
    ...
    let registry =
        crate::targets::load_registry_from_str(&serde_json::to_string(document)?).map_err(click)?;
    ...
    for existing in existing_stream {
        service::remove_service(document, target_name, existing.unit_id()).map_err(click)?;
    }
