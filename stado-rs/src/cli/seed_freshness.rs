...
pub fn classify(seed_state: &str, attempts: &[Attempt]) -> Verdict {
    ...
    if submitting.is_empty() {
        ...
        return if unreached > 0 {
            Verdict::PresentFailingElsewhere { attempts: unreached }
        } else {
...
pub fn attempts_of(evidence: &Value, login_item: &str) -> Vec<Attempt> {
    evidence
        ...
        .map(|rows| {
            rows.iter()
                .filter(|row| {
                    row.get("login_item").and_then(Value::as_str) == Some(login_item)
                })
                .map(|row| {
                    let flag = |name: &str| {
...
pub fn build_report(target: &str, vault: &Value, evidence: &Value) -> Value {
    ...
    findings.sort_by_key(|finding| {
        ...
        let urgent = finding
            ...
            .unwrap_or(false);
        (!urgent, finding.get("login_item").and_then(Value::as_str).unwrap_or_default().to_string())
    });
    json!({
        "schema_version": 1,
        "target": target,
...
pub fn parse_marked_line(stdout: &str, marker: &str, what: &str) -> Result<Value, CmdError> {
    ...
    serde_json::from_str(line)
        .map_err(|error| CmdError::click(format!("the {what} report is not readable JSON: {error}")))
}
...
pub fn render(report: &Value) -> String {
    ...
    for finding in findings {
        let item = finding
            ...
            .and_then(Value::as_str)
            .unwrap_or("?");
        let verdict = finding.get("verdict").and_then(Value::as_str).unwrap_or("?");
...
pub async fn authenticator_seed_freshness(
    ...
    let node = node.ok_or_else(|| {
        CmdError::click(format!("{}: Node.js is unavailable on this host", resolved.name))
    })?;
