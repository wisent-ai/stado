...
impl Tunnel {
    pub(crate) async fn open(paths: &[SshConnectionPath]) -> Result<Self, String> {
        ...
        Err(format!("no registry SSH connection path answered ({})", failures.join("; ")))
    }

    pub(crate) async fn connect(&self, host: &str, port: u16) -> Result<ChannelStream<client::Msg>, String> {
