...
mod tests {
    ...
    fn parse_inventory_ignores_a_chatty_login_shell() {
        let stdout = format!("Welcome to charless-mac-mini\n{}\n", incident_stdout());
        assert_eq!(
    ...
    /// it: `skarbiec-weles` declared on `19095` for `charless-mac-mini`,
    /// `skarbiec` on `8895`, plus `brama`, which the mini is only STANDING
    /// BY for — `lukasz-macbook` is its active host and the mini still
    ...
    fn mini_directory() -> ServiceDirectory {
        ...
        ServiceDirectory {
            authority: DirectoryAuthority {
                target: "charless-mac-mini".to_string(),
            ...
            services: BTreeMap::from([
                (
                    ...
                    service(
                        "charless-mac-mini",
                        &[("charless-mac-mini", "http://127.0.0.1:19095")],
                    ),
                ),
                (
                    "skarbiec".to_string(),
                    service(
                        "charless-mac-mini",
                        &[
                            ("charless-mac-mini", "http://127.0.0.1:8895"),
                            ("lukasz-macbook", "http://127.0.0.1:8787"),
                        ],
                    ),
                ),
                (
                    ...
                    service(
                        "lukasz-macbook",
                        &[
                            ("charless-mac-mini", "http://127.0.0.1:8080"),
                            ("lukasz-macbook", "http://127.0.0.1:9080"),
                        ],
                    ),
                ),
            ]),
    ...
    async fn a_marker_can_match_its_listener_and_still_disagree_with_the_registry() {
        ...
        // `brama` is ACTIVE on lukasz-macbook, which declares `9080`. This
    ...
    /// The other real host: `lukasz-macbook` runs `stado 0.4.392` while the
    ...
    async fn inventory_target_reports_drift_without_failing_the_command() {
        let report = inventory_target(&target(), None, &ok_runner(incident_stdout()))
            .await
            .unwrap();
        assert_eq!(report["target"], "charless-mac-mini");
    ...
    // -----------------------------------------------------------------
    // The sanitizer, and the incident it caused. On `charless-mac-mini` the
