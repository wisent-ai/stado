...
pub(super) fn merge(runtime: &mut ServeArgs, component: &InstallPlan) -> Result<(), DeployError> {
    ...
    let body = script
        .strip_prefix("while true; do ")
        .and_then(|body| body.strip_suffix("; done"))
        .ok_or_else(invalid)?;
    ...
    let (executable, options) = command
        .split_once(" scan-dispatch --execute ")
        .ok_or_else(invalid)?;
    ...
    if runtime
        .failure_fixer_interval_seconds
