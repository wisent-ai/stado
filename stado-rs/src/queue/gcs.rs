...
mod tests {
    ...
    #[test]
    fn percent_encode_keeps_unreserved_and_encodes_slash() {
        assert_eq!(percent_encode("a-._~b09Zz"), "a-._~b09Zz");
        assert_eq!(percent_encode("queue/j 1.json"), "queue%2Fj%201.json");
        // UTF-8 bytes are encoded per byte.
        assert_eq!(percent_encode("ż"), "%C5%BC");
    }

    #[test]
    fn upload_url_carries_name_and_generation_precondition() {
        assert_eq!(
            upload_url("bkt", "a/b", None),
            "https://storage.googleapis.com/upload/storage/v1/b/bkt/o?uploadType=media&name=a%2Fb"
        );
        assert_eq!(
            upload_url("bkt", "a/b", Some("0")),
            "https://storage.googleapis.com/upload/storage/v1/b/bkt/o?uploadType=media&name=a%2Fb&ifGenerationMatch=0"
        );
        assert_eq!(
            upload_url("bkt", "a/b", Some("177123")),
            "https://storage.googleapis.com/upload/storage/v1/b/bkt/o?uploadType=media&name=a%2Fb&ifGenerationMatch=177123"
        );
    }

    #[test]
    fn object_and_media_urls_encode_the_name_as_one_path_segment() {
        assert_eq!(
            object_url("bkt", "queue/j1.json"),
            "https://storage.googleapis.com/storage/v1/b/bkt/o/queue%2Fj1.json"
        );
        assert_eq!(
            media_url("bkt", "queue/j1.json"),
            "https://storage.googleapis.com/storage/v1/b/bkt/o/queue%2Fj1.json?alt=media"
        );
    }

    #[test]
    fn list_url_carries_prefix_fields_and_page_token() {
        assert_eq!(
            list_url("bkt", "queue/", None, "items(name),nextPageToken"),
            "https://storage.googleapis.com/storage/v1/b/bkt/o?prefix=queue%2F&fields=items%28name%29%2CnextPageToken"
        );
        assert_eq!(
            list_url("bkt", "queue/", Some("tok/en"), "items(name)"),
            "https://storage.googleapis.com/storage/v1/b/bkt/o?prefix=queue%2F&fields=items%28name%29&pageToken=tok%2Fen"
        );
    }

    #[test]
    fn parse_timestamp_reads_rfc3339() {
        let ts = parse_timestamp(&serde_json::json!("2026-05-16T12:34:56.789Z")).unwrap();
        assert_eq!(ts.to_rfc3339(), "2026-05-16T12:34:56.789+00:00");
        assert!(parse_timestamp(&serde_json::json!("junk")).is_none());
