...
fn checked_port(port: i64) -> Result<u16, ControlPlaneError> {
    u16::try_from(port)
        .map_err(|_| ControlPlaneError::Other(format!("port out of range: {port}")))
}
...
fn spawn_daemon<F>(name: &str, make_future: impl FnOnce() -> F + Send + 'static) -> std::io::Result<()>
where
...
async fn coordinator_loop(
    ...
    loop {
        ...
        match run_tick(&store, &secrets, &providers, with_billing, &|msg: &str| log(msg)).await {
...
pub async fn run_local(host: &str, port: i64, interval: i64) -> Result<(), ControlPlaneError> {
    ...
    spawn_daemon("stado-local-coordinator", move || {
        coordinator_loop(tick_store, BTreeMap::new(), interval.max(5) as u64, false, local_log)
    })?;
    ...
    Dashboard::new(store).serve_with(host, checked_port(port)?).await?;
    Ok(())
}
...
pub async fn run_cloud(host: &str, port: i64, interval: i64) -> Result<(), ControlPlaneError> {
    ...
    for key in ["HF_TOKEN", "HUGGING_FACE_HUB_TOKEN", "WC_SUPABASE_TOKEN", "WC_S3_BUCKET", "AWS_REGION"]
    {
    ...
    spawn_daemon("stado-cloud-coordinator", move || {
        coordinator_loop(tick_store, secrets, interval.max(15) as u64, true, cloud_log)
    })?;
    cloud_log(&format!("dashboard={host}:{port} storage={}", store.backend_name()));
    Dashboard::new(store).serve_with(host, checked_port(port)?).await?;
    Ok(())
}
