...
/// above still missed: `placement_profiles[].hosts[<host>].units` carries a
/// label, a path and a kind per service, and it is what `service remove`
/// refuses against — a profile whose unit has no managed record is a document
/// the validator will not accept, which is how a stale `services[]` record is
/// kept alive by a stale profile entry. So a doubled name here is load-bearing
/// twice over, and `com.wisent.compute.service.com.wisent.brama` sat in the
/// `brama-skarbiec` profile for lukasz-macbook, the fleet's own prefix minted
/// onto a name that already carried one, with no launchd job of that name on
/// the host at all.
///
/// No command edits a placement profile, so the remediation names the exact
/// key to correct rather than a command that does not exist.
