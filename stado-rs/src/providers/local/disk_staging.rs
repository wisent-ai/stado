...
/dev/nvme0n1p2 /data ext4 rw,relatime 0 0
/dev/sdb1 /slow ext4 ro,relatime 0 0
/dev/sdc1 /boot/efi vfat rw,relatime 0 0
overlay /var/lib/docker overlay rw,relatime 0 0
bad line
";

    #[test]
    fn parse_mounts_filters_pseudo_ro_and_boot() {
        let mounts = parse_mounts(MOUNTS);
        assert_eq!(mounts, vec!["/", "/data", "/var/lib/docker"]);
    }

    #[test]
    fn parse_mount_device_last_match_wins() {
        let text = "/dev/sda1 / ext4 rw 0 0\n/dev/sda2 / ext4 rw 0 0\n";
        assert_eq!(parse_mount_device(text, "/").as_deref(), Some("/dev/sda2"));
        assert_eq!(parse_mount_device(text, "/data"), None);
    }

    #[test]
    fn block_base_name_strips_partitions() {
        assert_eq!(block_base_name("nvme0n1p2"), "nvme0n1");
        assert_eq!(block_base_name("sda1"), "sda");
        assert_eq!(block_base_name("sda"), "sda");
        assert_eq!(block_base_name("mmcblk0p1"), "mmcblk0p");
    }

    #[test]
    fn rotational_resolution_against_fabricated_sys() {
        let dir = tempfile::tempdir().unwrap();
        let sys = dir.path();
        // Confirmed SSD: base dir exists with rotational=0.
        std::fs::create_dir_all(sys.join("block/nvme0n1/queue")).unwrap();
        std::fs::write(sys.join("block/nvme0n1/queue/rotational"), "0\n").unwrap();
        // Confirmed HDD.
        std::fs::create_dir_all(sys.join("block/sda/queue")).unwrap();
        std::fs::write(sys.join("block/sda/queue/rotational"), "1\n").unwrap();

        assert_eq!(resolved_block_base(sys, "nvme0n1"), "nvme0n1");
        // Partition name falls back to the stripped base.
        assert_eq!(resolved_block_base(sys, "nvme0n1p2"), "nvme0n1");
        assert!(!rotational_file_value(sys, "nvme0n1"));
        assert!(rotational_file_value(sys, "sda"));
        // Unknown device resolves to True (HDD), per the Python comment.
        assert!(rotational_file_value(sys, "missing"));
    }
    ...
    #[test]
    fn writable_for_self_probe() {
        let dir = tempfile::tempdir().unwrap();
        assert!(writable_for_self(dir.path()));
        // Probe file is removed afterwards.
        assert!(std::fs::read_dir(dir.path()).unwrap().next().is_none());
        assert!(!writable_for_self(Path::new("/nonexistent-wc-path")));
    }
}
