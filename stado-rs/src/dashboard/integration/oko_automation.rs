...
struct Host { host_id: String }
#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
struct Context { host_id: String, query: Option<String>, database: Option<String> }
#[derive(Deserialize)]
#[serde(rename_all = "lowercase")]
enum RoutineAction { Context, Autonomy }
#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
struct Create {
    host_id: String, id: String, name: String, action: RoutineAction,
    cron: String, time_zone: String, query: Option<String>, database: Option<String>,
}
#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
struct Routine { host_id: String, schedule_id: String }
#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
struct Run { host_id: String, schedule_id: String, retry_token: String }
#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
struct Inspect { host_id: String, job_id: String }
#[derive(Deserialize)]
#[serde(rename_all = "lowercase")]
enum Mode { Disabled, Experimental, Full }
#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
struct Policy { host_id: String, mode: Mode }

fn decode<T: DeserializeOwned>(body: &[u8]) -> Result<T, HandlerError> {
    serde_json::from_slice(body).map_err(|_| HandlerError::BadRequest)
}
fn words(parts: &[&str]) -> Vec<String> { parts.iter().map(|part| (*part).to_owned()).collect() }
fn identity(value: &str, prefix: &str) -> Result<(), HandlerError> {
    if value.strip_prefix(prefix).is_none_or(|suffix| suffix.is_empty()
...
fn optional(arguments: &mut Vec<String>, flag: &str, value: Option<String>) {
    if let Some(value) = value { arguments.extend([flag.to_owned(), value]); }
}

pub(super) fn supports(action: &str) -> bool {
    matches!(action, "context-snapshot" | "slack-status" | "autonomy-status" | "autonomy-set"
        | "routines-list" | "routines-create" | "routines-show" | "routines-pause"
        | "routines-resume" | "routines-remove" | "routines-run" | "routines-inspect")
}
    ...
    match action {
        ...
        "autonomy-set" => {
            ...
            let argv = match value.mode {
                ...
                Mode::Experimental => words(&["oko-cli", "autonomy", "enable", "--mode", "experimental"]),
        ...
        "routines-create" => {
            ...
            let mut argv = words(&["oko-cli", "routines", "create", "--id", &value.id,
                "--name", &value.name, "--action", match value.action {
                    RoutineAction::Context => "context", RoutineAction::Autonomy => "autonomy",
                }, "--cron", &value.cron, "--tz", &value.time_zone, "--host", &value.host_id]);
            optional(&mut argv, "--query", value.query);
            optional(&mut argv, "--db", value.database);
            Ok((value.host_id, argv))
        }
        "routines-show" | "routines-pause" | "routines-resume" | "routines-remove" => {
            ...
            let verb = action.strip_prefix("routines-").ok_or(HandlerError::BadRequest)?;
            Ok((value.host_id, words(&["oko-cli", "routines", verb, &value.schedule_id, "--json"])))
        }
        "routines-run" => {
            let value: Run = decode(body)?;
            identity(&value.schedule_id, "sch-")?;
            if value.retry_token.trim().is_empty() { return Err(HandlerError::BadRequest); }
            Ok((value.host_id, words(&["oko-cli", "routines", "run", &value.schedule_id,
                "--retry-token", &value.retry_token, "--json"])))
        }
        "routines-inspect" => {
            let value: Inspect = decode(body)?;
            identity(&value.job_id, "job-")?;
            Ok((value.host_id, words(&["oko-cli", "routines", "inspect", &value.job_id, "--json"])))
        }
...
pub(super) async fn handle(action: &str, body: &[u8]) -> HandlerResult {
    ...
    if arguments.iter().any(|argument| argument.contains('\0')) { return Err(HandlerError::BadRequest); }
    ...
    let output = crate::deploy::host_channel::run_program(&target, &program, &runner)
        .await.map_err(|_| HandlerError::UpstreamFailure)?;
    if output.stdout.len() > RESPONSE_LIMIT { return Err(HandlerError::ResponseTooLarge); }
    let data = serde_json::from_str::<Value>(&output.stdout);
    if output.ok() && data.is_err() { return Err(HandlerError::UpstreamFailure); }
    ...
    Ok(json!({"hostId": host, "operation": action, "exitStatus": output.code,
