...
use tokio_tungstenite::{
    tungstenite::{handshake::server, http, protocol::{Role, WebSocketConfig}, Message},
    WebSocketStream,
};

use super::{default_timeout, operator_auth, send_json, validate, Request, Response, RunRequest,
...
impl AttachmentRequest {
    fn command(self) -> Result<RunRequest, String> {
        ...
        for (flag, value) in [("--target", self.target), ("--workspace", self.workspace),
            ("--resume", self.resume)] {
            if let Some(value) = value { args.extend([flag.into(), value]); }
        }
        let request = RunRequest {
            args, input: None, stdin: None, confirmation: self.confirmation,
...
pub(crate) async fn upgrade(request: &Request) -> Response {
    if request.path != PATH || request.header("x-stado-action") != Some("workload-attach") {
        return send_json(STATUS_FORBIDDEN, &json!({"ok": false, "error": "forbidden"}));
    }
    match operator_auth::authorized(request).await {
        Ok(true) => {}
        Ok(false) => return send_json(STATUS_UNAUTHORIZED, &json!({"ok": false, "error": "unauthorized"})),
        Err(error) => return send_json(STATUS_UNAVAILABLE, &json!({"ok": false, "error": error.to_string()})),
    }
    if request.version != "HTTP/1.1" || !request.body.is_empty() {
        return send_json(STATUS_BAD_REQUEST, &json!({"ok": false, "error": "invalid WebSocket request framing"}));
    }
    let mut builder = http::Request::builder().method(request.method.as_str()).uri(&request.path);
    for (name, value) in &request.headers { builder = builder.header(name.as_str(), value.as_str()); }
    let response = builder.body(()).map_err(|error| error.to_string())
    ...
    bytes.extend_from_slice(b"\r\n");
    Response { status: response.status().as_u16(), bytes }
}
...
async fn setup(socket: &mut Socket) -> Result<Option<RunRequest>, String> {
    while let Some(message) = socket.next().await {
        match message.map_err(|error| error.to_string())? {
            ...
            Message::Ping(_) | Message::Pong(_) => socket.flush().await.map_err(|error| error.to_string())?,
...
pub(super) async fn error(socket: &mut Socket, detail: &str) {
    let _ = socket.send(Message::text(json!({"type": "error", "message": detail}).to_string())).await;
}
...
pub(crate) async fn serve(stream: TcpStream, carry: Vec<u8>) -> std::io::Result<()> {
    ...
    let mut socket = WebSocketStream::from_partially_read(stream, carry, Role::Server, Some(config)).await;
    let request = tokio::time::timeout(std::time::Duration::from_secs(default_timeout()), setup(&mut socket)).await;
    match request {
        Ok(Ok(Some(request))) => {
        ...
        Err(_) => error(&mut socket, "no attachment request arrived before the connection deadline").await,
    }
