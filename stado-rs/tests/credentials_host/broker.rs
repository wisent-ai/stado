//! The real Skarbiec broker these cases drive.
//!
//! No stand-in on PATH and no canned answer: the binary is either the one an
//! operator names in `SKARBIEC_BIN`, or the one built here from the sibling
//! Skarbiec checkout at `origin/main`. The source is taken with `git archive`
//! into an ignored cache keyed by commit, so the operator's own working tree
//! — which routinely carries uncommitted work — is read and never touched,
//! and a second run of this area reuses the build instead of repeating it.
...
const CACHE: &str = "../.wisent-output/credentials-host-skarbiec";
