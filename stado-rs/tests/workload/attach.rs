...
fn install_runtime(area: &Area) {
    ...
    let binary = fs::canonicalize(&binary).unwrap_or_else(|error| {
        panic!("blocked: the real installed Jeden binary {} cannot be read: {error}", binary.display())
    });
    let identity = Command::new(&binary).arg("--version").output()
        .expect("the real Jeden binary can start");
    assert!(identity.status.success(), "Jeden identity: {}", said(&identity.stderr));
...
fn attach(area: &Area, name: &str, resume: Option<&str>, requests: &[Value]) -> Output {
    ...
    let mut args = vec!["workload", "attach", "jeden-session", "--target", TARGET,
        "--workspace", "__home__"];
    ...
    let mut child = area.command(&args)
        ...
        .spawn().expect("the real attachment process starts");
    ...
    let status = loop {
        ...
        if Instant::now() >= deadline {
            let _ = child.kill();
            let _ = child.wait();
            panic!("attachment timed out; retained output: {}", directory.display());
        }
        std::thread::sleep(PROCESS_POLL);
    };
    fs::write(directory.join("result.json"),
        serde_json::to_vec(&json!({"exit_code": status.code()})).unwrap()).unwrap();
    Output { status, stdout: fs::read(stdout).unwrap(), stderr: fs::read(stderr).unwrap() }
}

fn reply(output: &Output, id: &str) -> Value {
    assert!(output.status.success(), "attachment failed: {}", said(&output.stderr));
    let response = said(&output.stdout).lines()
...
fn native_attachment_creates_a_real_ledger_and_refuses_a_missing_resume_ledger() {
    ...
    let opened = attach(&area, "create", None, &[
        json!({"id": "create", "method": "session/new", "params": {"cwd": home}}),
        json!({"id": "shutdown", "method": "shutdown"}),
    ]);
    let created = reply(&opened, "create");
    let ledger = Path::new(created["sessionPath"].as_str().expect("Jeden names its real ledger"));
    let ledger = fs::canonicalize(ledger).unwrap();
    assert!(ledger.starts_with(home.join(".jeden/sessions")), "Jeden wrote outside the isolated home");
    ...
    let transcript = fs::read(ledger.join("transcript.jsonl")).expect("Jeden persisted its event ledger");
    let session = ledger.file_name().unwrap().to_str().unwrap();
    let reconnected = attach(&area, "reconnect", Some(session), &[
    ...
    assert_eq!(fs::read(ledger.join("transcript.jsonl")).unwrap(), transcript);

    let missing = area.stado(&["workload", "attach", "jeden-session", "--target", TARGET,
        "--workspace", "__home__", "--resume", "absent-session"]);
    assert_eq!(missing.status.code(), Some(1));
    assert!(said(&missing.stderr).contains("session ledger is missing:"), "{}", said(&missing.stderr));
