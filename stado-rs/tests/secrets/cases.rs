...
fn identity_migration_refuses_missing_bearer_then_persists_valid_cutover() {
    ...
    let command = |args: &[&str]| {
        fixture
            .command(std::path::Path::new(env!("CARGO_BIN_EXE_stado")))
            .env("STADO_CONFIG", &path)
            .args(args)
            .output()
            .expect("run Stado config CLI")
    };
    assert_success(&command(&["config", "init"]), "initialize isolated config");
    let mut document: Value = serde_json::from_slice(&fs::read(&path).unwrap()).unwrap();
    document["credentials"]["admin"] =
        serde_json::json!({"consumer":"old-admin", "token_file":"old-token"});
    document["object_api"]["skarbiec"] =
        serde_json::json!({"consumer":"old-object", "token_file":"old-object-token"});
    document["secrets"]["skarbiec"]["token_file"] =
        Value::from(fixture.token.to_string_lossy().as_ref());
    fs::write(
        &path,
    ...
    assert_eq!(
        fs::read(&path).unwrap(),
        before,
        "refusal changed the config"
    );

    fixture.grant_username();
    assert_success(
        &command(&["config", "migrate-identities"]),
        "migrate identity config",
    );
    ...
    assert_eq!(
        fs::read(format!("{}.before-identity-migration", path.display())).unwrap(),
        before
    );
    assert_success(
