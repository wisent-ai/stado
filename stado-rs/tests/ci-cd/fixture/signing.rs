...
pub(crate) fn seed_native_signing_input(home: &Path, storage: &Path) {
    let selected = Command::new(env!("CARGO_BIN_EXE_stado"))
        .args(["config", "show"])
        .output()
        .expect("the real Stado configuration reader runs");
    assert!(
    ...
    let namespace = std::env::var("WC_STADO_STORAGE_NAMESPACE")
        ...
        .unwrap_or_else(|| {
            ...
            declared["storage"]["stado"]["namespace"]
                .as_str()
                .filter(|value| !value.is_empty())
                .expect("the selected Stado configuration declares no signing-input namespace")
                .to_owned()
        });
...
fn seed_pinned_input(home: &Path, storage: &Path, namespace: &str, leaf: &str, sha: &str) {
    ...
    let fetched = Command::new(env!("CARGO_BIN_EXE_stado"))
        ...
        .arg(&fetched_copy)
        .output()
        .expect("the Stado object reader runs");
    ...
    fs::write(
        home.join("native-issuer-fetch.exit.txt"),
        fetched.status.to_string(),
    )
    .unwrap();
    assert!(
        fetched.status.success(),
        "blocked: the pinned Apple issuer {uri} could not be read from the real store: {}",
        String::from_utf8_lossy(&fetched.stderr)
    );
    let (_, digest) = stado::release_control::sha256_file(&fetched_copy).unwrap();
    assert_eq!(
        digest, sha,
        "the real store served another Apple issuer chain"
    );
