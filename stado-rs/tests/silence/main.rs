...
//! `charless-mac-mini` stopped answering at 18:29 UTC and came back at
...
const HOST: &str = "charless-mac-mini";
...
async fn a_silence_opens_on_the_crossing_and_closes_on_the_fresher_beacon() {
    ...
    let merged = observe_beacon_age_at(
        ...
        300,
        READER_CLI,
        Some("beacon for charless-mac-mini is 350s old"),
    )
    .await
    .unwrap()
...
async fn refusals_aggregate_per_reason_over_a_window() {
    ...
    for record in [
        ...
        refusal(
            ...
            READER_CLI,
            REASON_BEACON_STALE,
            "beacon for charless-mac-mini is 281s old",
        ),
    ] {
    ...
    assert_eq!(listed[0].detail, "beacon for charless-mac-mini is 281s old");
    assert_eq!(listed[3].at, at("2026-08-19T18:30:11Z"));

    // A host nobody refused about answers zero, not an error.
    let none = refusal_summary_at(&store, "lukasz-macbook", 900, now)
        .await
        .unwrap();
