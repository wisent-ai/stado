...
fn a_failed_host_read_keeps_completed_store_reads_and_never_invents_disk_pressure() {
    let evidence_root = Path::new(env!("CARGO_MANIFEST_DIR")).join(".wisent-output/host-diagnostics");
    fs::create_dir_all(&evidence_root).unwrap();
    let evidence = tempfile::Builder::new().prefix("gates-").tempdir_in(evidence_root).unwrap().keep();
    ...
    let host = Host::new();
    fs::copy(host.storage.join("registry.json"), evidence.join("registry-before.json")).unwrap();

    let measured = run(&host, &evidence, "measured");
    assert_eq!(measured["complete"], true, "{measured}");
    let free = measured["disk"]["free_bytes"].as_i64().expect("a completed host read reports measured bytes");
    let native = df_root().available_kb * KIB;
    fs::write(evidence.join("native-free-bytes.txt"), native.to_string()).unwrap();
    assert!((free - native).abs() < MEASUREMENT_DRIFT_BYTES, "reported {free}, measured {native}");
    assert!(host.janitor_state().is_none(), "a diagnostic read must not run cleanup");
    ...
    assert!(partial["claiming"].is_null(), "an incomplete diagnosis cannot decide admission: {partial}");
    ...
    assert_eq!(observation(&partial, "disk_usage")["state"], "error", "{partial}");
    assert_eq!(observation(&partial, "queue")["state"], "complete", "{partial}");
    assert_eq!(observation(&partial, "capacity")["state"], "absent", "{partial}");
    assert!(!partial["blockers"].as_array().unwrap().iter().any(|value|
        matches!(value.as_str(), Some("disk_pressure_active" | "disk_pressure_unresolved"))));
    let persisted: Value = serde_json::from_slice(&fs::read(&path).unwrap()).unwrap();
    assert_eq!(persisted, registry, "a read must not repair the declaration");
    ...
    assert_eq!(observation(&unavailable, "registry")["state"], "error", "{unavailable}");
    assert_eq!(observation(&unavailable, "disk_usage")["state"], "skipped", "{unavailable}");
...
fn observation<'a>(report: &'a Value, operation: &str) -> &'a Value {
    report["observations"].as_array().unwrap().iter()
        .find(|value| value["operation"] == operation).expect("named diagnostic reading")
}
...
fn run(host: &Host, evidence: &Path, step: &str) -> Value {
    ...
    fs::write(evidence.join(format!("{step}.process.json")), serde_json::to_vec_pretty(&json!({
        "command": [env!("CARGO_BIN_EXE_stado"), "host", "gates", TARGET, "--json"],
        "exit_code": output.status.code(), "success": output.status.success(),
    })).unwrap()).unwrap();
    assert_eq!(output.status.code(), Some(REFUSED), "{}", String::from_utf8_lossy(&output.stderr));
