...
fn archived_executable_digest(path: &Path) -> String {
    ...
    for entry in archive.entries().unwrap() {
        ...
        if entry.path().unwrap() != Path::new("wisent-products") { continue; }
        assert!(digest.is_none(), "the actual SDK archive contains an ambiguous executable");
        assert!(entry.header().entry_type().is_file(), "the qualified SDK entry is not a regular executable");
...
fn qualified_sdk_bytes_and_source_claim_cannot_drift() {
    ...
    let fetched = journey.stado(&[
        "release", "fetch", "wisent-products", stado::deploy::native_signing::runtime::VERSION,
        "--platform", platform, "--source-commit", source,
        "--destination", archive.to_str().unwrap(), "--json",
    ]);
    assert!(fetched.status.success(), "the native SDK cannot be independently verified against its signed release: {}", stderr(&fetched));
    let (_, actual) = stado::release_control::sha256_file(&program).unwrap();
    assert_eq!(actual, archived_executable_digest(&archive), "Stado did not install the qualified SDK's actual executable bytes");

    let mut changed = receipt.clone();
    let wrong_source = "0".repeat(source.len());
    assert_ne!(source, wrong_source, "the negative source claim must differ from the actual release");
    ...
    assert!(!refused.status.success(), "a declared source replaced executable provenance");
    let diagnostic = stderr(&refused);
    assert!(diagnostic.contains(source) && diagnostic.contains(&wrong_source),
        "the source refusal omitted the expected or observed source: {diagnostic}");
    assert_eq!(stado::release_control::sha256_file(&program).unwrap().1, actual,
        "a refused source claim changed the installed native SDK");
    fs::write(&record, &original).unwrap();

    OpenOptions::new().append(true).open(&program).unwrap().write_all(b"changed-sdk-bytes").unwrap();
    let (_, changed_digest) = stado::release_control::sha256_file(&program).unwrap();
    assert_ne!(changed_digest, actual);
    let refused = journey.stado(&["product", "catalog", "--json"]);
    assert!(!refused.status.success(), "the product command succeeded after the managed SDK bytes changed");
    let diagnostic = stderr(&refused);
    assert!(diagnostic.contains(&actual) && diagnostic.contains(&changed_digest),
