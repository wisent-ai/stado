...
impl Journey {
    pub fn new() -> Self {
        ...
        let compiled_source =
            option_env!("STADO_SOURCE_REVISION").or(option_env!("WISENT_SOURCE_COMMIT"));
        ...
        assert!(
            compiled_source
                .is_some_and(|source| source.len() == 40
        ...
        assert!(
            ...
            "cannot read the actual selected Stado configuration: {}",
            stderr(&output)
        );
        let selected: Value = serde_json::from_slice(&output.stdout).unwrap();
        journey.config = Some(PathBuf::from(
            selected["file"]
                .as_str()
                .expect("stado config show did not name its selected file"),
        ));
        let catalog = journey.catalog();
        journey.retain(
            "initial-catalog.json",
    ...
    fn run(&self, mut command: Command) -> Output {
        ...
        let output = match command.stdin(Stdio::null()).output() {
            ...
            Err(error) => {
                ...
                panic!(
                    "real command could not start; evidence {}: {error}",
                    directory.display()
                );
            }
        };
    ...
    pub fn stado(&self, args: &[&str]) -> Output {
        ...
        command
            ...
            .env(
                ...
                self.config
                    .as_ref()
                    .expect("selected configuration is required"),
            )
    ...
    pub fn catalog(&self) -> Value {
        ...
        assert!(
            output.status.success(),
            "qualified native product catalog is unavailable: {}",
            stderr(&output)
        );
    ...
    pub fn assert_no_installation(&self) {
        ...
        match fs::read_dir(&records) {
            ...
            Err(error) => panic!(
                "cannot inspect actual product records {}: {error}",
                records.display()
            ),
        }
        for directory in [".stado/bin", ".local/bin", "Applications"] {
            assert!(
                !self.home.join(directory).try_exists().unwrap(),
                "a product installation destination was created: {directory}"
            );
        }
    }
    ...
    pub fn finish(mut self) {
        self.root
            ...
            .unwrap()
            .close()
            .expect("consumer cleanup failed");
        self.retain(
            "outcome.json",
            &serde_json::to_vec_pretty(&json!({
    ...
    fn drop(&mut self) {
        if !self.finished {
            ...
            if let Err(error) = fs::write(self.evidence.join("outcome.json"), outcome.to_string()) {
                eprintln!(
                    "cannot retain failed native product journey {}: {error}",
