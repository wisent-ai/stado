...
//! covers the bytes it was signed over and not what follows them.
...
pub const HOLD: &str = "STADO_STALE_IMAGE_HOLD_SECONDS";
...
const PROBE: &str = "stale_image_probe_child";
...
const MARKER: &[u8] = b"\n# stado stale-unit-image replacement marker\n";
...
impl LoadedUnit {
    ...
    pub fn start(host: &Host, nickname: &str) -> Self {
        ...
        .expect("copy the test binary into the fixture");
        ...
        assert!(
            ...
            unit.domain,
            said(&bootstrap.stdout),
            said(&bootstrap.stderr)
        );
        unit.loaded = true;
...
pub fn label(nickname: &str) -> String {
    format!(
        "{}stado-test.stale-image-{}-{nickname}",
...
/// Push a file's modification time back by `seconds`.
pub fn backdate(path: &Path, seconds: i64) {
    let when = SystemTime::now()
        - Duration::from_secs(u64::try_from(seconds).expect("a positive age in seconds"));
    std::fs::File::options()
        .write(true)
        .open(path)
        .expect("open the file to retime it")
        .set_times(std::fs::FileTimes::new().set_modified(when))
        .expect("retime the file");
}
