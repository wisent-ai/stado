//! What the Apple challenge stories talk to: the real endpoint under test,
//! the request each story sends, and the readers that judge the answer.
...
pub(crate) const APPLE_ONLY_PLAN: &str = r#"{"schema":"wisent.gui-automation-plan.v1",
...
pub(crate) const API_COMMAND_SECONDS: u32 = 300;
pub(crate) const COMMAND_SECONDS: u64 = 360;
...
impl Report {
    pub(crate) fn state(&self) -> BTreeMap<&str, &str> {
        self.items
            .iter()
    ...
    pub(crate) fn unrelated_state(&self) -> BTreeMap<&str, &str> {
        let mut state = self.state();
        state.retain(|key, _| !key.starts_with("apple-challenge-"));
        state
    }

    pub(crate) fn assert_ready(&self, target: &str) {
...
pub(crate) fn plan_file() -> PathBuf {
...
pub(crate) async fn run(args: &[&str]) -> Output {
...
pub(crate) fn report(output: &Output) -> Report {
...
pub(crate) async fn status(target: &str) -> Report {
    report(
        &run(&[
            "workload",
            "status",
...
pub(crate) async fn prepare(target: &str, plan: &str) -> Output {
    let verbs = ["workload", "run", "gui-automation"];
    run(&[&verbs[..], &["--target", target, "--plan", plan, "--json"]].concat()).await
}

pub(crate) fn registered_host() -> String {
