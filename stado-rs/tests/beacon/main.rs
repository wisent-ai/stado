...
/// `gpu-host`; the suspend lines carry systemd's own
...
fn publish_beacon_leaves_a_relayed_document_without_this_hosts_link() {
    ...
    let published = document(&fixture.collect(&fixture.beacon("control-host")));
    assert_eq!(published["host"], "control-host");
    assert_eq!(
        published.get("link"),
        None,
