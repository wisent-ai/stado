//! The real Skarbiec broker the beacon publication chain needs.
//!
//! `stado host publish-beacon` sends the merged document to the Stado
//! host-health route, and that route compares the presented bearer against
//! `stado-host-health-api/token` read out of a vault through the object
//! verifier grant. Nothing about that can be stood in for, so the broker here
//! is either the binary an operator names in `SKARBIEC_BIN` or the one built
//! from the sibling Skarbiec checkout at `origin/main`. The source is taken
//! with `git archive` into an ignored cache keyed by commit, so the
//! operator's own working tree is read and never touched and a second run of
//! this area reuses the build.
...
const CACHE: &str = "../.wisent-output/beacon-skarbiec";
