...
impl NativeApi {
    async fn start() -> Self {
        ...
        fs::write(
            &config,
            json!({"storage": {"backend": "local", "local": {"path": storage}}}).to_string(),
        )
        .unwrap();
        fs::write(
            storage.join("registry.json"),
            json!({"schema_version": 2, "targets": [], "coordinators": []}).to_string(),
        )
        .unwrap();
        ...
        loop {
            assert!(
                api.child.try_wait().unwrap().is_none(),
                "real Stado server exited; evidence {}",
                api.root.display()
            );
            if let Some(address) = fs::read_to_string(&log)
                .unwrap()
                .lines()
            ...
            assert!(
                ...
                "real Stado server did not listen; evidence {}",
                api.root.display()
            );
    ...
    fn cli(&self, name: &str, args: &[&str]) -> std::process::Output {
        let output = Self::command(&self.root, &self.home, &self.config)
            .args(args)
            .output()
            .unwrap();
        ...
        fs::write(
            self.root.join(format!("{name}.process.json")),
            json!({"args": args, "exit_code": output.status.code()}).to_string(),
        )
        .unwrap();
        output
    }
    ...
    async fn request(&self, name: &str, body: Value) -> (u16, Value) {
        let response = reqwest::Client::new()
            .post(format!("{}/api/operator/run", self.address))
        ...
        fs::write(
            ...
            json!({"request": body, "http_status": status, "response": payload}).to_string(),
        )
        .unwrap();
        (status, payload)
    }
...
async fn native_input_writes_only_after_confirmation_and_is_read_back_independently() {
    ...
    let (status, refusal) = api
        .request("unconfirmed", json!({"args": args, "stdin": CONTENT}))
        .await;
    ...
    assert_eq!(
        serde_json::from_slice::<Value>(&before.stdout).unwrap()["state"],
        "absent"
    );
    ...
    let downloaded = api.cli(
        "read-back",
        &["storage", "get", OBJECT, destination.to_str().unwrap()],
    );
    assert!(
    ...
    assert_eq!(
        serde_json::from_slice::<Value>(&after.stdout).unwrap()["state"],
        "absent"
    );
}
