//! The persistent verifier bearer, end to end against the real vault: the
//! first mint, a byte-identical reuse, the owner-only mode it keeps, and the
//! two refusals — a symlinked token file and an empty one — that must leave
//! the vault exactly as they found it.
...
use crate::helpers::digest;

/// Every read here is of the real vault the fixture created, and the two
/// refusals must leave it byte-identical.
pub(crate) fn verify_persistent_verifier(
    fixture: &DashboardFixture,
    verifier_token: &Path,
    verifier_token_json: &str,
