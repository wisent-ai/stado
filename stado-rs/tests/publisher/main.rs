...
//!
//! [`runner`] holds the refusals the publisher runner lifecycle produces before
//! it touches any machine, which run by default. [`developer_id`] holds the one
//! case that issues a Developer ID on a dedicated macOS host, which cannot run
//! without Account Holder consent at an Apple prompt and is gated with the
