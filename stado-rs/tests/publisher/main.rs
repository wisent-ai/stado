...
fn required(name: &str) -> String {
    std::env::var(name).unwrap_or_else(|_| panic!("{name} must name the dedicated publisher fixture"))
}
...
fn publisher_runner_install_reconciles_an_existing_runner_without_org_admin_access() {
    ...
    let installed = success(&run(&[
        "host", "publisher-runner", "install", &target,
    ...
    assert!(installed["stdout"].as_str().unwrap_or_default().contains("runner service: running"));

    let status = success(&run(&[
        "host", "publisher-runner", "status", &target, "--json",
    ]));
...
fn developer_id_issues_once_reuses_the_bundle_and_grants_repository_signing() {
    ...
    let issued = success(&run(&[
        "host", "publisher-runner", "developer-id", &target,
        "--account-item", &account_item,
        "--repository", &repository,
        "--json",
    ]));
    assert!(matches!(issued["status"].as_str(), Some("issued" | "reused")));
    assert!(issued["identity"].as_str().unwrap_or_default().starts_with("Developer ID Application:"));
    ...
    let secret_names = Command::new("gh")
        .args(["secret", "list", "--repo", &format!("wisent-ai/{repository}"), "--json", "name"])
        .output()
        .expect("gh starts");
    assert!(secret_names.status.success(), "gh secret list failed: {}", String::from_utf8_lossy(&secret_names.stderr));
    let names: serde_json::Value = serde_json::from_slice(&secret_names.stdout).expect("gh returns JSON");
    for required in ["MACOS_CERT_P12", "MACOS_CERT_PASSWORD", "MACOS_SIGN_IDENTITY"] {
        assert!(names.as_array().unwrap().iter().any(|row| row["name"] == required), "missing {required}");
    }
}
