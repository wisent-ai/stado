...
- **Launchd definition convergence:** `service ensure` now compares the desired Program and argument vector with launchd's retained definition, not only the plist. A stale retained definition is reloaded only after executable and plist preflight, and success requires launchd readback plus the running executable to match.
