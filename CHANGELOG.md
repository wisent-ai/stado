...
- **Neutral storage evidence:** exact-object backup audits now name the two physical roots `local_storage` and `local_backup` in JSON (and `local-storage`/`local-backup` in text), include effective metadata-sidecar identities, and report the immediate `ecosystem/` directory children from both roots with an explicit completeness bit. Repeated `--inventory-namespace` scopes then list only backup-visible object paths and size metadata without reading object bodies, so the physical namespace inventory can be bounded before exact hashes are requested. The diagnostic no longer labels either physical set as authoritative; broad replica classification and reclamation semantics are unchanged.
- **Exact copy identity:** backend copies now skip an existing destination object only when its bytes equal the source, rather than treating equal byte length as identity. A same-length stale lifecycle or control document is overwritten by the designated source before mirror pruning or migration verification can proceed.
- **Durable queued cancellation:** each coordinator tick now completes cancellation markers whose original caller stopped after recording the request but before moving the queued job. Finalization uses the existing version-pinned lifecycle transition, preserves the marker and its original `requested_at`, and runs independently of dispatch eligibility so a marked job cannot remain queued forever.
- **Legacy run retention:** before strict v3 validation or any cleanup handling, the run reaper retains and explicitly reports unsupported legacy manifests. It neither fabricates terminal outcomes nor deletes legacy manifests or lifecycle evidence.
- **Release capacity precondition:** the self-hosted runner first invokes the installed product's declared host reclamation, then separately clears this checkout's Cargo output because it is outside every host-reclaim root. The host's own gate must measure free space at or above its declared target before the exact release revision compiles; after compilation the candidate completes the declared reclamation, immutable coordinates are claimed, the private Cargo target is cleared under `always()`, and the capacity measurement is repeated. A bounded reclamation's successful exit is no longer mistaken for proof that it freed enough space.
- **Cleanup budget fairness:** every declared janitor cleaner now receives both an item share and a wall-clock share from the single bounded pass, with unused budget rolling forward in the existing fixed order. The complete order includes `release_store`, so a whole-home build-cache walk or backup-twin hashing cannot consume the time or item budget reserved for later lifecycle cleanup.
- **Migration and rollback:** no stored data changes occur during installation. Install a compatible release before the reviewed storage handoff; rolling back removes exact-byte copy identity, neutral root evidence, queued-cancellation settlement, and fail-closed legacy-run retention.
- **Platforms and evidence:** the supported native platforms remain `darwin-arm64` and `linux-amd64`; exact-head qualification and immutable release publication supply the repository's declared source and delivery evidence.
