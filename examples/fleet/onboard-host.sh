...
#
# Prerequisites on the target, all of them before this script: it is reachable
# at <ssh-destination> (any destination that opens — a .local name on the same
# network or a tailnet name both work), Remote Login is enabled, and the fleet's
# public key is in its ~/.ssh/authorized_keys. Enrollment probes the machine over
# that key before it writes anything, so there is no order in which this script
# can create the channel it needs. Here: a reachable `skarbiec serve`, and
# SKARBIEC_VAULT_FILE
